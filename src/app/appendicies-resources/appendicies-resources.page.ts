...
@Component({
  selector: 'app-appendicies-resources',
  templateUrl: './appendicies-resources.page.html',
  styleUrls: ['./appendicies-resources.page.scss'],
})
export class AppendiciesResourcesPage implements OnInit {
  ...
  renderResourceData(){
    ...
    if(this.network.type == "none"){
      let localBookList = localStorage.getItem("downloadedBooks");
      let bookslist = (localBookList!== null ) ? JSON.parse(localBookList) : null;
  ...
  getBookByBarcode(barcodeInfo) {
    ...
    if (barcodes) {
      ...
      if (barcodes[barcodeInfo["text"]] == undefined) {
        ...
        return this.gotoappendices(bookID, 'appendices');
      }
    } else {
      this.presentLoading();
      const route =
